...
In most scenarios, the Primetime Authentication API include additional error information in the response body in order to provide **meaningful context** to why a certain error has occurred and/or possible solutions to automatically remediate the issue.  *However, in some specific cases involving authentication or logout flows, the Primetime Authentication services might return an HTML response or an empty body - please check the API documentation for more information.*
...
The Primetime Authentication API returns HTTP Status codes in the range 400-500 to indicate failures or errors. Each HTTP status code has certain implications:
...
Adobe Primetime APIs which handle multiple items (Preauthorization API,etc) might indicate if the processing has failed for a particular item and was successful for other items by using item-level error information. In this case, the ***"error"*** object is located at the item level and the response body might contain multiple ***"errors"*** objects - please consult the API documentation.
...
|Code|*string*|network_connection_failure|♦|The standard Primetime Authentication error code. The complete list of error codes is included below.|
...
||*unsupported_client_version* | 400 |  This version of Primetime Authentication SDK is too old and is no longer supported. Please check the API documentation for the steps needed to upgrade to the latest version.|
