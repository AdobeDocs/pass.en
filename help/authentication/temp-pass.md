...
*   **Temp Pass access is granted to individual users on specific devices**. After Temp Pass access expires for a user, that user will not be able to get temporary access on the same device until that user's expired [authorization token](/help/authentication/glossary.md#authz-token) is cleared from the Adobe Pass Authentication server.
