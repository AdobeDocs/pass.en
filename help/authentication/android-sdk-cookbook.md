...
The Adobe Primetime authentication entitlement solution for Android is ultimately divided into two domains:
...
1.  Initiate Adobe Primetime authentication
    ...
    a.  Call [`getInstance`](#$getInstance) to create a single instance of the Adobe Primetime authentication AccessEnabler.
        
      - **Dependency:** Adobe Primetime authentication Native
        Android Library (AccessEnabler)
    
    b.  Call` setRequestor()` to establish the identify of the Programmer; pass in the Programmer's `requestorID` and (optionally) an array of Adobe Primetime authentication endpoints.
        
      - **Dependency:** Valid Adobe Primetime authentication RequestorID  
        (Work with your Adobe Primetime authentication Account Manager to arrange this.)
...
1.  Validate the Short Media Token.  
    Use the Adobe Primetime authentication Media Token Verifier library to to verify the short-lived media token returned from the `getAuthorization()` call above:
