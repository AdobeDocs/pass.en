...
The media token is a token generated by Adobe Pass Authentication as a result of an authorization decision meant to provide viewing access to protected content (resource). The media token is valid for a limited and short timeframe (a few minutes) specified at the moment of issue, indicating the amount of time it must be used by the client application before requiring to retrieve a new one.
...
* The `isValid()` method used to validate the media token. It accepts a single argument, the [resource identifier](/help/authentication/integration-guide-programmers/features-standard/entitlements/protected-resources.md). If the provided resource identifier is `null`, the method will validate only the media token's authenticity and validity period.
...
Refer to the **Response** and **Samples** sections of the above API to understand the authorization decision and media token models.
