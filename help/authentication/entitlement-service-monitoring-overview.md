...
| unique-sessions | Number of unique sessions that performed authentication flow calls to the Adobe Primetime authentication service within the selected time interval. (This metric will only show if daily values are requested.) </br> This is computed for each individual Data Center. When "dc" dimension is not requested, this metric will not be displayed. | 
...
| eap | The external authentication provider when authentication flow is performed through an external system. </br> The values can be: </br> - N/A - the authentication was provided by Primetime authentication </br> - Apple - the external system that provided the authentication is Apple | 
| os-family | Operating system running on the device | 
| browser-family | User agent used for accessing Adobe Primetime authentication | 
...
| event | The Adobe Primetime authentication event name | 
| reason | The reason for failures, as reported by Adobe Primetime authentication | 
...
- **Monitoring** - Ops or monitoring teams can create a dashboard or chart that calls the API every minute. Using the information displayed they can detect a problem (with Primetime authentication, or with an MVPD) the minute it appears.  
