...
1. The user then starts a third stream from the Programmer's website.
1. The rule in the policy (max 2 streams, latest wins) allows the latest stream to play so **the first stream that was started will be marked as non-compliant with the policy and will be stopped.**
